...
internal sealed class BoardRepository(ScrumBoardDbContext dbContext) : IBoardRepository
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            tasksQuery = tasksQuery.Where(item => EF.Functions.ILike(item.task.Title, $"%{filter.Search}%") ||
                (item.task.Description != null && EF.Functions.ILike(item.task.Description, $"%{filter.Search}%")));
        }
