...
internal sealed class IdempotencyRecordRowConfiguration : IEntityTypeConfiguration<IdempotencyRecordRow>
{
    public void Configure(EntityTypeBuilder<IdempotencyRecordRow> builder)
    {
        ...
        builder.Property(record => record.ResponseBody).HasColumnName("response_body").HasColumnType("text");
        builder.Property(record => record.Location).HasColumnName("location").HasMaxLength(500);
        builder.Property(record => record.Etag).HasColumnName("etag").HasMaxLength(100);
        builder.Property(record => record.BoardEtag).HasColumnName("board_etag").HasMaxLength(100);
        ...
        builder.HasIndex(record => new { record.UserId, record.Key })
            .IsUnique().HasDatabaseName("ux_idempotency_user_key");
