...
{
    public async Task<BoardSnapshot> GetAsync(Guid projectId, BoardFilter filter, CancellationToken cancellationToken)
    {
    ...
    public async Task<ColumnResponse> CreateColumnAsync(Guid projectId, CreateColumn request, CancellationToken cancellationToken)
    {
        ...
        await notifier.PublishAsync(projectId, "ColumnChanged", response, cancellationToken);
        return response;
    }
    ...
    public async Task<ColumnResponse> MoveColumnAsync(
        Guid projectId,
        Guid columnId,
        MoveColumn request,
        ...
        CancellationToken cancellationToken)
    {
        ...
        project.TouchBoard(clock.UtcNow);
        await unitOfWork.SaveChangesAsync(cancellationToken);
        await notifier.PublishAsync(projectId, "ColumnChanged", new { columnId, deleted = true, project.BoardVersion }, cancellationToken);
    }

    public async Task<TaskResponse> CreateTaskAsync(Guid projectId, CreateTask request, CancellationToken cancellationToken)
    {
        ...
        await notifier.PublishAsync(projectId, "TaskCreated", response, cancellationToken);
        return response;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        await unitOfWork.SaveChangesAsync(cancellationToken);
        var response = ToResponse(task, project.BoardVersion);
        await notifier.PublishAsync(projectId, "TaskUpdated", response, cancellationToken);
        return response;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        await unitOfWork.SaveChangesAsync(cancellationToken);
        var response = ToResponse(task, project.BoardVersion);
        await notifier.PublishAsync(projectId, "TaskMoved", response, cancellationToken);
        return response;
    }
    ...
    public async Task DeleteTaskAsync(Guid projectId, Guid taskId, long expectedVersion, CancellationToken cancellationToken)
    {
        ...
        project.TouchBoard(clock.UtcNow);
        await unitOfWork.SaveChangesAsync(cancellationToken);
        await notifier.PublishAsync(projectId, "TaskDeleted", new { taskId, project.BoardVersion }, cancellationToken);
    }

    public async Task<IReadOnlyList<BoardMember>> GetMembersAsync(Guid projectId, CancellationToken cancellationToken)
    {
        var board = await GetAsync(projectId, new BoardFilter(), cancellationToken);
    ...
    private static BoardFilter Normalize(BoardFilter filter) => filter with { Search = filter.Search?.Trim() };
    private static void EnsureVersion(long actual, long expected)
    {
        if (actual != expected) throw new PreconditionFailedException("etag_mismatch", "The resource changed after it was read.");
    }

    private static ColumnResponse ToResponse(BoardColumn column, long boardVersion) =>
